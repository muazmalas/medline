...
class Session {
  ...
  Future<bool> restore() async {
    ...
    try {
      api.token = token;
      user = (jsonDecode(storedUser) as Map).cast<String, dynamic>();
      api.setCacheNamespace('${user?['id'] ?? user?['email'] ?? 'anonymous'}');
  ...
  Future<bool> rotateAccessToken() {
    ...
    _rotationInFlight = operation.whenComplete(() {
  ...
  Future<void> signIn(String email, String password,
    ...
    final response =
        await api.login(email, password, twoFactorCode: twoFactorCode);
