...
export function OrderDetailPanel({ detail, onClose, locale }: { detail: Record<string, unknown>; onClose: () => void; locale: string }) {
  ...
  return <section className="content order-detail-content">
    ...
    <div className="order-detail-grid">
      <section className="panel invoice-card"><div className="panel-heading"><div><p className="eyebrow">{locale === 'ar' ? 'الفاتورة' : 'INVOICE'}</p><h2>{locale === 'ar' ? 'ملخص الطلب' : 'Order summary'}</h2></div><span className="detail-total">{amount('total')}</span></div><div className="invoice-lines"><p><span>Subtotal</span><strong>{amount('subtotal')}</strong></p><p><span>Delivery fee</span><strong>{amount('delivery_fee')}</strong></p><p className="invoice-grand-total"><span>Total</span><strong>{amount('total')}</strong></p></div><div className="payment-line"><span>Payment method</span><strong className="payment-pill"><CreditCard size={14} />{paymentMethod}</strong></div></section>
