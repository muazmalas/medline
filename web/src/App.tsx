...
function DriverProfilePanel({ driverId, onClose }: { driverId: number; onClose: () => void }) {
...
export function OperationsPage({ section, role, locale }: { section: string; role: string; locale: string }) {
  ...
  const richHeaders = section === 'procurement' ? ['Procurement', 'Pharmacy', 'Warehouse', 'Value', 'Status', 'Created', 'Action'] : section === 'inventory' ? ['Medicine', 'Owner', 'Available', 'Reserved', 'Unit price', 'Stock health'] : ['Order', 'Customer', 'Pharmacy', 'Driver', 'Medicines', 'Destination', 'Total', 'Status', 'Action']
  const richCells = (row: Row) => section === 'procurement' ? <><strong>{row.primary}</strong><span>{String(row.raw?.pharmacy_name ?? 'Pharmacy not recorded')}</span><span>{String(row.raw?.warehouse_name ?? 'Warehouse not recorded')}</span><span className="money-cell">SYP {Number(row.raw?.total ?? 0).toLocaleString()}</span><span className="status-pill">{row.status}</span><span>{formatMedlineDate(row.raw?.created_at)}</span>{action(row)}</> : section === 'inventory' ? <><strong>{row.primary}</strong><span>{String(row.raw?.owner_name ?? row.raw?.owner_type ?? 'Owner not recorded')}</span><span>{String(Number(row.raw?.quantity ?? 0) - Number(row.raw?.reserved_quantity ?? 0))}</span><span>{String(row.raw?.reserved_quantity ?? 0)}</span><span className="money-cell">SYP {Number(row.raw?.unit_price ?? 0).toLocaleString()}</span><span className={`stock-health ${Number(row.raw?.quantity ?? 0) - Number(row.raw?.reserved_quantity ?? 0) <= Number(row.raw?.low_stock_threshold ?? 0) ? 'low' : 'healthy'}`}>{Number(row.raw?.quantity ?? 0) - Number(row.raw?.reserved_quantity ?? 0) <= Number(row.raw?.low_stock_threshold ?? 0) ? 'Low stock' : 'Healthy'}</span></> : <><strong>{row.primary}</strong><span>{String(row.raw?.customer_name ?? 'Customer not recorded')}</span><span>{String(row.raw?.pharmacy_name ?? 'Pharmacy not recorded')}</span><span>{String(row.raw?.driver_name ?? 'Unassigned')}</span><span>{String(row.raw?.medicine_names ?? 'No medicines listed')}</span><span>{String(row.raw?.delivery_address_snapshot ?? 'Destination not recorded')}</span><span className="money-cell">SYP {Number(row.raw?.total ?? 0).toLocaleString()}</span><span className="status-pill">{row.status}</span>{action(row)}</>
  ...
  return <section className="content"><div className="welcome-row"><div><p className="eyebrow">MEDLINE OPERATIONS</p><h1>{tr(section, locale)}</h1><p className="muted">{tr('workflowGuidance', locale)}</p></div>{section === 'inventory' && role !== 'admin' && <form className="inline-form" onSubmit={updateInventory}><input name="medicine_id" type="number" placeholder={tr('medicineId', locale)} required /><input name="quantity" type="number" placeholder={tr('quantity', locale)} min="0" required /><input name="unit_price" type="number" placeholder={tr('unitPrice', locale)} min="0" step="0.01" /><input name="low_stock_threshold" type="number" placeholder={tr('lowStock', locale)} min="0" defaultValue="5" /><button className="primary-button" type="submit"><Package size={17} /> {tr('saveStock', locale)}</button></form>}</div>{section === 'complaints' && role === 'admin' && complaintReport && <section className="metric-grid"><Metric label="Open complaints" value={String(reportTotals.open ?? 0)} change="Live" icon={<MessageSquare />} tone="orange" /><Metric label="In review" value={String(reportTotals.in_review ?? 0)} change="Live" icon={<History />} tone="violet" /><Metric label="Resolved complaints" value={String(reportTotals.resolved ?? 0)} change="Live" icon={<ShieldCheck />} tone="green" /></section>}<section className={`panel table-panel ${richTable ? 'rich-operations-panel' : ''}`}><div className="panel-heading"><div><p className="eyebrow">{tr(['orders', 'deliveries', 'verification', 'procurement'].includes(section) ? 'queue' : 'directory', locale)}</p><h2>{tr(section, locale)} {tr('overview', locale)}</h2></div><span className="live-status" role="status"><i aria-hidden="true" /> {tr('liveData', locale)}</span></div><div className="search-box"><Search size={19} aria-hidden="true" /><input aria-label={`${tr('search', locale)} ${tr(section, locale)}`} value={search} onChange={(event) => setSearch(event.target.value)} placeholder={`${tr('search', locale)} ${tr(section, locale)}...`} /></div><div className="operations-table" aria-busy={loading}><div className="table-row table-head">{richTable ? richHeaders.map((header) => <span key={header}>{header}</span>) : <><span>{tr('record', locale)}</span><span>{tr('details', locale)}</span><span>{tr('status', locale)}</span><span>{tr('action', locale)}</span></>}</div>{loading ? <div className="state" role="status" aria-live="polite">{tr('loadingRecords', locale)}</div> : rows.length === 0 ? <div className="state" role="status">{tr('noRecordsYet', locale)}</div> : rows.map((row) => <div className={`table-row ${richTable ? 'rich-table-row' : ''} ${canOpenDetail ? 'clickable-table-row' : ''}`} key={row.id} onClick={() => { if (canOpenDetail) void openDetail(row.id) }} onKeyDown={(event) => { if (canOpenDetail && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); void openDetail(row.id) } }} tabIndex={canOpenDetail ? 0 : undefined}>{richTable ? richCells(row) : <><strong>{row.primary}</strong><span>{row.secondary}</span><span className="status-pill">{row.status}</span>{action(row)}</>}</div>)}</div></section></section>
}
