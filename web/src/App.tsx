...
function RegistrationPage({ onBack, onAuthenticated }: { onBack: () => void; onAuthenticated: (user: Record<string, unknown>, token: string) => void }) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(''); setMessage(''); setLoading(true); try { const response = await api.post('/auth/register', { ...form, role, transport: 'cookie' }); localStorage.setItem('medline_token', response.data.token); localStorage.removeItem('medline_refresh_token'); setMessage(response.data.message ?? 'Registration submitted.'); onAuthenticated(response.data.user ?? {}, response.data.token) } catch (requestError) { setError(axios.isAxiosError(requestError) ? requestError.response?.data?.message ?? 'Unable to submit registration.' : 'Unable to submit registration.') } finally { setLoading(false) } }
