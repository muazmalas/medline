...
function RegistrationPage({ onBack, onAuthenticated }: { onBack: () => void; onAuthenticated: (user: Record<string, unknown>, token: string) => void }) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(''); setMessage(''); document.querySelectorAll('.registration-invalid').forEach((input) => input.classList.remove('registration-invalid')); setLoading(true); try { const response = await api.post('/auth/register', { ...form, role, transport: 'cookie' }); localStorage.setItem('medline_token', response.data.token); localStorage.removeItem('medline_refresh_token'); setMessage(response.data.message ?? 'Registration submitted.'); onAuthenticated(response.data.user ?? {}, response.data.token) } catch (requestError) { if (axios.isAxiosError(requestError)) { const data = requestError.response?.data as { message?: string; errors?: Record<string, string[]> } | undefined; const errors = data?.errors ?? {}; highlightErrors(errors); const details = Object.entries(errors).map(([field, messages]) => `${field.replaceAll('_', ' ')}: ${messages.join(' ')}`).join(' · '); setError(details || data?.message || 'Unable to submit registration.') } else setError('Unable to submit registration.') } finally { setLoading(false) } }
