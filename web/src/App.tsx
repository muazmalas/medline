...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {section === 'dashboard' ? <><LiveDashboard role={role} locale={locale} /><DashboardAlerts role={role} locale={locale} /><NotificationHealthPanel role={role} locale={locale} /></> : section === 'subscriptions' && role !== 'admin' ? <PartnerSubscriptionPage locale={locale} /> : section === 'settings' ? <>{role === 'admin' ? <><SettingsPage role="patient" locale={locale} onLocaleChange={setLocale} /><AdminTwoFactorPanel locale={locale} /></> : <SettingsPage role={role} locale={locale} onLocaleChange={setLocale} />}<ConsentSettings /></> : section === 'inventory' && role === 'admin' ? <><MedicineAdminPage locale={locale} /><MedicineEditAdminPage locale={locale} /><MedicineCategoryAdmin locale={locale} /></> : (section === 'pharmacies' || section === 'warehouses') && role === 'admin' ? <PartnerManagementPanel section={section} /> : section === 'users' && role === 'admin' ? <UserRolePanel section={section} /> : <OperationsPage section={section} role={role} locale={locale} />}
    </main>
  </div>
}
