...
export function OperationsPage({ section, role, locale }: { section: string; role: string; locale: string }) {
  ...
  const richTable = section === 'procurement' || section === 'inventory'
  const richHeaders = section === 'procurement' ? ['Procurement', 'Pharmacy', 'Warehouse', 'Value', 'Status', 'Created'] : ['Medicine', 'Owner', 'Available', 'Reserved', 'Unit price', 'Stock health']
  const richCells = (row: Row) => section === 'procurement' ? <><strong>{row.primary}</strong><span>{String(row.raw?.pharmacy_name ?? 'Pharmacy not recorded')}</span><span>{String(row.raw?.warehouse_name ?? 'Warehouse not recorded')}</span><span className="money-cell">SYP {Number(row.raw?.total ?? 0).toLocaleString()}</span><span className="status-pill">{row.status}</span><span>{formatMedlineDate(row.raw?.created_at)}</span></> : <><strong>{row.primary}</strong><span>{String(row.raw?.owner_name ?? row.raw?.owner_type ?? 'Owner not recorded')}</span><span>{String(Number(row.raw?.quantity ?? 0) - Number(row.raw?.reserved_quantity ?? 0))}</span><span>{String(row.raw?.reserved_quantity ?? 0)}</span><span className="money-cell">SYP {Number(row.raw?.unit_price ?? 0).toLocaleString()}</span><span className={`stock-health ${Number(row.raw?.quantity ?? 0) - Number(row.raw?.reserved_quantity ?? 0) <= Number(row.raw?.low_stock_threshold ?? 0) ? 'low' : 'healthy'}`}>{Number(row.raw?.quantity ?? 0) - Number(row.raw?.reserved_quantity ?? 0) <= Number(row.raw?.low_stock_threshold ?? 0) ? 'Low stock' : 'Healthy'}</span></>
