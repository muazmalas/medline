...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {section === 'dashboard' ? <><LiveDashboard role={role} locale={locale} /><DashboardAlerts role={role} locale={locale} /><NotificationHealthPanel role={role} locale={locale} /></> : section === 'subscriptions' && role !== 'admin' ? <PartnerSubscriptionPage locale={locale} /> : section === 'settings' ? <>{role === 'admin' ? <><SettingsPage role="patient" locale={locale} onLocaleChange={setLocale} /><AdminTwoFactorPanel locale={locale} /></> : <SettingsPage role={role} locale={locale} onLocaleChange={setLocale} />}<ConsentSettings /></> : section === 'inventory/categories' && role === 'admin' ? <><InventoryBackLink /><MedicineCategoryAdmin locale={locale} /></> : section === 'inventory' && role === 'admin' ? <><InventoryCategoryLink /><MedicineAdminPage locale={locale} /><MedicineEditAdminPage locale={locale} /></> : (section === 'pharmacies' || section === 'warehouses') && role === 'admin' ? <PartnerManagementPanel section={section} /> : section === 'users' && role === 'admin' ? <UserRolePanelWithCompany section={section} /> : <OperationsPage section={section} role={role} locale={locale} />}
    </main>
  </div>
}
...
function TablePagination({ page, lastPage, onPageChange, status, onStatusChange }: { page: number; lastPage: number; onPageChange: (page: number) => void; status?: string; onStatusChange?: (status: string) => void }) {
  if (lastPage <= 1 && !onStatusChange) return null
  return <nav className="table-pagination" aria-label="Table pagination">{onStatusChange && <label className="status-filter-label">Filter status<select value={status ?? ''} onChange={(event) => onStatusChange(event.target.value)}><option value="">All statuses</option><option value="pending_pharmacy_review">Pending pharmacy review</option><option value="prescription_review">Prescription review</option><option value="accepted">Accepted</option><option value="in_transit">In transit</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></label>}<button type="button" className="ghost-button" disabled={page <= 1} onClick={() => onPageChange(page - 1)}>Previous</button><span>Page <strong>{page}</strong> of <strong>{lastPage}</strong></span><button type="button" className="ghost-button" disabled={page >= lastPage} onClick={() => onPageChange(page + 1)}>Next</button></nav>
}
