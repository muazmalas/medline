...
export function PartnerManagementPanel({ section }: { section: string }) {
  ...
  const load = async () => { try { const response = await api.get('/admin/partners', { params: { type: partnerType, per_page: 100 } }); setPartners(response.data.data ?? []) } catch { setPartners([]) } }
  useEffect(() => { if (section === 'pharmacies' || section === 'warehouses') void load() }, [section])
